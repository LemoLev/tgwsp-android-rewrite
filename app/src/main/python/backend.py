    ...
    try:
        async with server:
            if _server_stop_event:
                ...
                stop_task = asyncio.create_task(_server_stop_event.wait())
                ...
                for task in pending:
                    task.cancel()
                    try:
                        await task  # Даем задаче завершиться после отмены
                    except asyncio.CancelledError:
                        pass
...
def main(argss, stop_signal: asyncio.Event, loop: Optional[asyncio.AbstractEventLoop] = None):
    ...
    try:
        dc_redirects = parse_dc_ip_list(args.dc_ip[0])
    ...
    print("start proxy")
    try:
        loop.run_until_complete(_run(stop_signal))
